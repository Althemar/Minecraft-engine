...
class NYCamera
{
	public:
		...
		NYVert3Df _FwdRef; /// Devant la caméra ( pas là ou elle regarde)
		...
		void updateVecs(void)
		{
			...
			_FwdRef = _UpRef.vecProd(_NormVec);
		}
		...
		{
			float angleCamera = acos(_Direction.scalProd(_FwdRef));
			angleCamera = 180 * angleCamera / M_PI;
			if (_UpRef.scalProd(_Direction) < 0)
				angleCamera = -angleCamera;
				
			if (!( angleCamera > 89 && angle > 0 ) && !( angleCamera < -89  && angle < 0))
			{
				_LookAt -= _Position;
		...
		{		
			//On ne monte pas trop hautpour ne pas passer de l'autre coté
			...
			if (!( angleCamera > 80 && angle > 0 ) && !( angleCamera < -80 && angle < 0 ))
			{
				cout << angleCamera << "       " << angle << endl;
