...
public:
	...
	YVec2 & operator= (const YVec2 & vertex)
	{
		X = vertex.X;
		Y = vertex.Y;
	...
	YVec2 operator * (YVec2 vert) const
	{
		YVec2 temp(X * vert.X,
			Y * vert.Y);
		return temp;
	...
	YVec2 operator * (T scalaire) const
	{
		YVec3 temp(X * scalaire,
			Y * scalaire);
		return temp;
	...
	YVec2 operator / (T scalaire) const
	{
		YVec2 temp(X / scalaire,
			Y / scalaire);
		return temp;
	}

	YVec2 operator + (const YVec2 & vertex) const
	{
		YVec3 temp(X + vertex.X,
			Y + vertex.Y);
		return temp;
	...
	bool operator== (const YVec2 & vertex) const
	{
	...
	///Le produit scalaire (dot product)
	T dot(const YVec2 & vertex) const
	{
		return vertex.X*X + vertex.Y*Y;
	}

	///Le produit vectoriel
	YVec2 cross(const YVec2 & vertex) const
	{
		return a.x * b.y - a.y * b.x;

		YVec3 produit(X * vertex.Y - Y * vertex.X);

		return produit;
	}

	// DO NOT USE
	YVec2 & rotate(YVec2 & axe, float angle)
	{
